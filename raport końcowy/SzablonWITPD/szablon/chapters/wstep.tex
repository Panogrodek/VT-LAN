...
szyfrowanie transmisji oparte na algorytmie AES-256-GCM oraz uwierzytelnianie połączeń.
...
Pozostała część raportu zorganizowana jest następująco. W dalszych sekcjach
niniejszego rozdziału przedstawiono zastosowane technologie, zakres wykorzystania
narzędzi AI, analizę opłacalności i użyteczności społecznej projektu oraz przegląd
rozwiązań dostępnych na rynku (sekcja~\ref{sec:przeglad_rynku}).
Rozdział~\ref{chapt:Backend} opisuje warstwę backendową aplikacji: podstawy
teoretyczne kodeka Opus i biblioteki GNS, architekturę klient-serwer, tor transmisji
głosowej wraz z mechanizmem VAD, system RPC, przesyłanie plików oraz bezpieczeństwo
transmisji. Rozdział~\ref{ch:interfejs_uzytkownika} przedstawia interfejs
użytkownika - jego założenia projektowe, ewolucję w trakcie realizacji projektu
oraz funkcje finalnej wersji aplikacji. Rozdział~\ref{ch:profilowanie_debugging}
dokumentuje proces profilowania, diagnostyki i kontroli jakości, w tym testy
skalowalności i weryfikację warstwy kryptograficznej.
Rozdział~\ref{chapt:Zakonczenie} podsumowuje zrealizowane prace, przedstawia
podział zadań w zespole oraz instrukcję obsługi aplikacji.
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		realizowane jest przy użyciu algorytmu AES-256-GCM per pakiet,
...
na poziomie aplikacji (AES-256-GCM) dodatkowo chroni przesyłane treści.
...
Discord \cite{Discord} to jedna z najpopularniejszych platform komunikacyjnych,
...
zewnętrznej ani kont użytkowników. Zestawienie cech omawianych rozwiązań
przedstawiono w tabeli~\ref{tab:rynek_porownanie}.

\begin{table}[H]
	\centering
	\caption{Porównanie komunikatorów dostępnych na rynku}
	\label{tab:rynek_porownanie}
