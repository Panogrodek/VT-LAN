...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|p{4.2cm}|p{6.2cm}|}
		...
		\textbf{UDP (reliable)} & \texttt{udp.port == 27020}      & Weryfikacja narzutu nagłówków dla czatu tekstowego i systemu RPC -- kanał niezawodny GNS (TCP-like). \\ \hline
		\textbf{UDP (unreliable)} & \texttt{udp \&\& ip.addr == 192.168.1.0/24} & Badanie ciągłości strumienia audio kodeka Opus \cite{OpusRFC} oraz poprawności działania algorytmu VAD -- kanał bezgwarancyjny GNS. \\ \hline
	\end{tabular}
\end{table}
...
\begin{itemize}
	...
	integracji backendu sieciowego z warstwą kryptograficzną (AES-256-GCM
...
uruchomionych instancji procesu \texttt{Audio Diagnostics.exe}
(nazwa procesu dla aplikacji VT-LAN w trybie testowym) zarejestrowano
przy użyciu systemowego Menedżera Zadań.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Testy skalowalności i weryfikacja końcowa}
\label{sec:db_skalowalnosc}
% ─────────────────────────────────────────────────────────────────────────────

Po wdrożeniu biblioteki SDL\_mixer \cite{SDLMixer} jako warstwy miksującej
strumienie audio oraz refaktoryzacji mechanizmu buforowania po stronie
odbiorczej, przeprowadzono pełen cykl testów weryfikacyjnych w celu
potwierdzenia, że aplikacja VT-LAN poprawnie obsługuje docelową liczbę
uczestników. Wszystkie opisane poniżej testy zakończyły się wynikiem
pozytywnym.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Test wytrzymałościowy -- 12 uczestników z transferem pliku}
\label{subsec:db_test_12}
% ─────────────────────────────────────────────────────────────────────────────

Scenariusz testowy zakładał uruchomienie 12 niezależnych instancji
aplikacji VT-LAN na jednej maszynie lokalnej w architekturze pętli
zwrotnej (\textit{localhost}). Jedna instancja pełniła rolę hosta,
pozostałe jedenaście działały jako klienci. Zakres testu obejmował
kanał niezawodny GNS (TCP-like) - weryfikowano poprawność jednoczesnego
przesyłania wiadomości tekstowych i dużego pliku do wszystkich instancji
klienckich.

Transmisja głosowa nie była objęta tym testem z przyczyn architektonicznych
wynikających z charakteru środowiska jednomaszynowego. Wszystkie instancje
uruchomione na jednym urządzeniu współdzielą to samo fizyczne urządzenie
wejściowe audio - mikrofon systemowy. W konsekwencji każda instancja
przechwytywałaby i nadawała identyczny sygnał, co uniemożliwiłoby
miarodajną weryfikację miksowania niezależnych strumieni głosowych
ani zachowania mechanizmu VAD w warunkach równoczesnej aktywności
wielu rozmówców. Test głosowy na architekturze \textit{localhost}
pozwoliłby jedynie potwierdzić poprawność pętli zwrotnej jednej instancji,
nie dostarczając żadnych danych o rzeczywistej skalowalności podsystemu audio.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\textwidth]{figures/12_instancji.png}
	\caption{Test wytrzymałościowy aplikacji VT-LAN: 12 instancji uruchomionych
		na jednej maszynie (host + 11 klientów localhost) podczas
		jednoczesnego przesyłania wiadomości tekstowych i dużego pliku
		(praca własna)}
	\label{fig:db_test_wytrzymalosciowy}
\end{figure}

Wyniki testu (\cref{fig:db_test_wytrzymalosciowy}) potwierdziły poprawność
działania kanału danych w scenariuszu obciążeniowym:

\begin{itemize}
	\item \textbf{Transfer pliku (kanał GNS reliable -- TCP-like)} --
	plik został dostarczony do wszystkich jedenastu instancji
	klienckich w nienaruszonym stanie. Mechanizm rate-limitingu
	segmentów opisany w sekcji~\ref{sec:tcp} skutecznie zapobiegł
	przepełnieniu buforów nadawczych GNS, a pasek postępu
	po stronie klientów odzwierciedlał rzeczywisty stan transferu.
	\item \textbf{Komunikacja tekstowa (kanał GNS reliable -- TCP-like)} --
	wiadomości czatu dostarczane były do wszystkich instancji
	w poprawnej kolejności bez utraty danych, niezależnie od
	jednoczesnego transferu pliku.
	\item \textbf{Stabilność sesji} -- przez cały czas trwania testu
	żadna z instancji klienckich nie utraciła połączenia,
	a mechanizm \texttt{ConnectionMessageFlag} poprawnie zarządzał
	rozgłaszaniem do wszystkich 11 odbiorców.
\end{itemize}

Wybór liczby 12 instancji był podyktowany możliwościami sprzętowymi
stanowiska testowego i pozwolił na wstępną weryfikację skalowalności
mechanizmów sieciowych projektu. Sesja 12-osobowa nie stanowi docelowego
limitu aplikacji - architektura GNS i system flag bitowych
\texttt{ConnectionMessageFlag} nie wprowadzają sztywnego ograniczenia
liczby uczestników.

Dodatkowym argumentem przemawiającym za pominięciem osobnych testów
wytrzymałościowych dla transmisji głosowej jest istotna różnica narzutu
danych między kanałami. Pakiet głosowy kodeka Opus ma stały rozmiar
rzędu 151\,B, podczas gdy transfer pliku generuje strumień segmentów
o wielokrotnie większym łącznym wolumenie danych. Skoro mechanizmy
sieciowe aplikacji poprawnie obsłużyły jednoczesne rozgłaszanie dużego
pliku do 11 klientów - scenariusz znacznie bardziej obciążający szynę
sieciową - transmisja głosowa, jako ruch lżejszy i buforowany przez
mechanizm VAD w stanach ciszy, powinna skalować się między uczestnikami
bez problemów.

Wartościowym kierunkiem dalszych badań byłoby przeprowadzenie testów
na rzeczywistej sieci LAN z większą liczbą fizycznych urządzeń,
co umożliwiłoby jednoczesną weryfikację skalowalności kanału głosowego,
zachowania mechanizmu VAD przy wielu równoczesnych nadawcach oraz
stabilności miksowania w bibliotece SDL\_mixer \cite{SDLMixer}.
Testy tego rodzaju wymagają jednak skoordynowanego zaangażowania
odpowiednio licznego zespołu testowego i infrastruktury sieciowej
wykraczającej poza ramy niniejszego projektu.
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Test komunikacji między różnymi urządzeniami w sieci LAN}
\label{subsec:db_test_lan}
% ─────────────────────────────────────────────────────────────────────────────

Uzupełnieniem testów skalowalności był test interoperacyjności,
weryfikujący poprawność komunikacji pomiędzy dwoma fizycznie różnymi
maszynami podłączonymi do tej samej sieci lokalnej. Celem było
potwierdzenie, że aplikacja poprawnie obsługuje rzeczywiste adresy IP
sieci LAN, inicjalizację połączenia GNS przez \texttt{ConnectByIPAddress}
oraz pełen dwukierunkowy przepływ danych między różnymi konfiguracjami
sprzętowymi.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\textwidth]{figures/urzadzenie_1.png}
	\caption{Test komunikacji między dwoma różnymi urządzeniami
		w sieci lokalnej: widok ekranu hosta (praca własna)}
	\label{fig:db_test_dwa_urzadzenia_host}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\textwidth]{figures/urzadzenie_2.png}
	\caption{Test komunikacji między dwoma różnymi urządzeniami
		w sieci lokalnej: widok ekranu klienta (praca własna)}
	\label{fig:db_test_dwa_urzadzenia_klient}
\end{figure}

Test przeprowadzono na dwóch maszynach o różnych konfiguracjach
sprzętowych, zestawionych bezpośrednio przez przełącznik sieciowy.
Wyniki (\cref{fig:db_test_dwa_urzadzenia_host}
i~\cref{fig:db_test_dwa_urzadzenia_klient}) potwierdziły:

\begin{itemize}
	\item \textbf{Poprawność nawiązania połączenia} --
	klient prawidłowo zlokalizował hosta po adresie IP i porcie,
	a handshake GNS zakończył się sukcesem wywołując callback
	\texttt{ConnectionStateConnected} po obu stronach sesji.
	\item \textbf{Dwukierunkowa transmisja głosowa (UDP)} --
	pakiety głosowe kodeka Opus przepływały prawidłowo w obu
	kierunkach; oba urządzenia rejestrowały i odtwarzały dźwięk
	bez zakłóceń, a mechanizm VAD redukował ruch w stanach ciszy
	na obu stanowiskach niezależnie.
	\item \textbf{Dwukierunkowa komunikacja tekstowa i transfer pliku
		(GNS reliable -- TCP-like)} -- wiadomości czatu docierały do
	obu uczestników w poprawnej kolejności i bez utraty danych,
	a przesłane pliki zachowały integralność po złożeniu
	segmentów przez \texttt{TryAssembleLocked}.
	\item \textbf{Synchronizacja listy uczestników} -- po dołączeniu
	klienta host i klient widzieli identyczną listę uczestników,
	co potwierdza poprawność działania mechanizmu RPC
	\texttt{NewPlayerSync} i \texttt{AddPlayerSync} opisanego
	w sekcji~\ref{sec:tcp}.
\end{itemize}

% ─────────────────────────────────────────────────────────────────────────────
\section{Wnioski i podsumowanie prac diagnostycznych}
\label{sec:db_wnioski}
% ─────────────────────────────────────────────────────────────────────────────

Wykryta usterka audio jednoznacznie wskazała granice skalowalności
wczesnej wersji prototypowej aplikacji VT-LAN. Jako rekomendację naprawczą
wskazano konieczność wprowadzenia warstwy miksującej audio - zostało to
zrealizowane przez wdrożenie biblioteki SDL\_mixer \cite{SDLMixer},
co potwierdziły późniejsze testy skalowalności.

Przeprowadzone działania z zakresu profilowania, diagnostyki oraz kontroli
jakości pozwoliły na rzetelną ocenę kondycji technicznej aplikacji VT-LAN:

\begin{itemize}
	\item potwierdzono bardzo niskie zużycie zasobów systemowych:
	RAM nieprzekraczający 100\,MB na instancję (typowo ok.\ 70\,MB),
	obciążenie CPU na poziomie ok.\ 1\%,
	\item wykazano wysoką efektywność sieciową mechanizmu VAD --
	ponad 60\% redukcji pasma sieciowego w stanach ciszy,
	\item udowodniono poprawność integracji warstwy kryptograficznej
	AES-256-GCM przez pełną obfuskację zawartości pakietów UDP,
	\item potwierdzono poprawność kanału niezawodnego (TCP-like) przy
	12 równoczesnych instancjach localhost -- transfer pliku
	i komunikacja tekstowa zakończyły się pomyślnie bez utraty danych,
	\item potwierdzono poprawność dwukierunkowej komunikacji głosowej (UDP)
	i tekstowej między dwoma różnymi urządzeniami w sieci LAN,
	\item sesja 12-osobowa stanowi wstępny punkt odniesienia dla
	skalowalności, nie zaś docelowy limit aplikacji -- architektura
	GNS i system flag bitowych \texttt{ConnectionMessageFlag}
	nie narzucają sztywnego ograniczenia liczby uczestników,
	\item dalszy rozwój projektu powinien obejmować testy na rzeczywistej
	sieci LAN z większą liczbą fizycznych urządzeń, umożliwiające
	empiryczną weryfikację skalowalności transmisji głosowej
	przy większej liczbie równoczesnych nadawców.
\end{itemize}
